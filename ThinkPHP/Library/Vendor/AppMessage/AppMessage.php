...
class AppMessage {
	...
	//发送Android单播消息
	function sendAndroidUnicast($device_id,$title,$content,$redirect_url) {
		try {
			...
			$unicast->setPredefinedKeyValue("device_tokens",    $device_id); 
			...
			// Set extra fields
			$unicast->setExtraField("url", $redirect_url);
	...
	//发送IOS单播消息
	function sendIOSUnicast($device_id,$content,$redirect_url) {
		try {
			...
			// Set customized fields
			$unicast->setCustomizedField("redirect_url", $redirect_url);
			//print("Sending unicast notification, please wait...\r\n");
			$unicast->send();
			//print("Sent SUCCESS\r\n");
		} catch (Exception $e) {
...
// $demo = new AppMessage("your appkey", "your app master secret");
