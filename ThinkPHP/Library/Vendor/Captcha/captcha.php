...
class Captcha
{
     ...
     * @var string  $folder
     */
    var $folder     = '/Public/captcha/';
	/**
	 * 
	 */
    var $savepath = '/Upload/Api/captcha/';
    /**
     * 图片的文件类型
     *
     * @var string  $img_type
    ...
    {
    	$folder || $this->folder = $_SERVER['DOCUMENT_ROOT'].$this->folder;
    ...
    function generate_image($word = false)
    {
       ...
       // $this->record_word($word);
        ...
        //指定黄色背景tao
        //$theme = array('captcha_bg6.jpg','255','255','255');
        ...
        else
        {
            ...
            if(!is_dir($this->savepath)) @mkdir($_SERVER['DOCUMENT_ROOT'].$this->savepath);
    		$filepath = $this->savepath.substr(md5(time()),16).'.png';
            imagepng($img_org,$_SERVER['DOCUMENT_ROOT'].$filepath);
            imagedestroy($img_org);
            return array('code'=>$word,'codepath'=>'http://'.$_SERVER['HTTP_HOST'].$filepath);
        }
    }
