...
// function add_opt_log($adminer,$content,$type=1){
// 	if(!empty($adminer) && !empty($content)){
// 		return D('OptLog')->add(array('adminer_name'=>$adminer,'content'=>$content,'type'=>$type));
// 	}
// }
/**
 * 设置客服
 ...
 */
// function set_service($user_id,$province_id=''){
    // if(empty($user_id))return false;
    // $user = D('Users')->find($user_id);
    // if(empty($user)) return false;
    // if($province_id){//分配区域客服
        // $services = D('Service')->where(array('type'=>'3','status'=>'1','province_id'=>array('like','%,'.$province_id.',%')))->order('create_date asc')->select();
    // }else{//分配注册客服
        // $services = D('Service')->where(array('type'=>'2','status'=>'1'))->order('create_date asc')->select();
    // }
    // if(empty($services))return false;
    // $min = null;
    // $min_data = null;
    // foreach($services as $val){
        // $num = D('Users')->where(array('service_id'=>$val['id']))->count();
        // if($min === null){
            // $min = $num;
            // $min_data = $val;
        // }else{
    ...
    // }
    // if($min_data){
        // return D('Users')->where(array('id'=>$user_id))->save(array('service_id'=>$min_data['id'])) ? true : false;
    // }else{
        // return false;   
    // }
// }
function set_service($user_id,$province_id=''){
    ...
    if(empty($user)) return false;
    if($province_id){//分配区域客服
        $where = array('type'=>'3','status'=>'1','seek'=>'1','province_id'=>array('like','%,'.$province_id.',%'));
    }else{//分配注册客服
        $where = array('type'=>'2','status'=>'1','seek'=>'1');
    }
    $service = D('Service')->where($where)->order('id asc')->find();
    unset($where['seek']);
    if(empty($service)){
        $service = D('Service')->where($where)->order('id asc')->find();
    }else{
        $max_id = D('Service')->where($where)->order('id desc')->getField('id');
        if($max_id == $service['id']){
            $service = D('Service')->where($where)->order('id asc')->find();
        }else{
            $service = D('Service')->where(array_merge($where,array('id'=>array('GT',$service['id']))))->order('id asc')->find();
        }
    }
    if($service){
        D('Service')->where(array('id'=>$service['id']))->save(array('seek'=>1));//将指针移到当前客服
        D('Service')->where(array_merge($where,array('id'=>array('NEQ',$service['id']))))->save(array('seek'=>0));//将指针从其他客服那儿移走
        return D('Users')->where(array('id'=>$user_id))->save(array('service_id'=>$service['id'])) ? true : false;
    }else{
        return false;
    }
}
 ...
 * 添加推荐记录
 * @param $rec_data 推荐数据
 * @param $invite_type  推荐方式：1、扫码推荐 2、推荐好友 
 */
 function add_rec_log($rec_data,$invite_type = 1){
     if(!$rec_data) return false;
     D('RecommendLog')->add(array(
        'inviter_type'=>$rec_data['inviter_type'],
        'invite_type'=>$rec_data['invite_type'],
        'inviter_id'=>$rec_data['inviter_id'],
        'invitee_id'=>$rec_data['invitee_id'],
        'get_money'=>$invite_type == '1' ? $rec_data['get_money'] : $rec_data['friend_money']
     ));
 }
?>