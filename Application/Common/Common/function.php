 ...
 * @param $num  商品的数量
 */
function getFavrate ($cart_id,$num)
{
    $where = "id = ".$cart_id;
    $goods = M("cart") ->where($where)-> find();
    $total_price = $goods["goods_price"] * $num;

    $prom_order = M("prom_order") -> where("id = ".$goods["prom_id"]) -> find();

    if($total_price >= $prom_order["money"] && $prom_order)
    {
        if ($prom_order["type"] == 0)
        {
            $prom_price = $total_price -  ($total_price * ($prom_order["expression"]/100));
        }elseif($prom_order["type"] == 1)
        {
            $prom_price =  $prom_order["expression"];
        }
    }
    return $prom_price;
...
{
    $config = M("config") -> where("(name = 'freight_free' or name = 'base_shipping') and inc_type = 'shopping'") -> getField("name,value");
    return $config;
}
...
*/
function checkSMS($to, $datas, $tempId, $parseFlag = true){
    $accountSid   = "8aaf070859b4c993015a06e6246b0982";
    $accountToken = "eccd7c33f4094005bf82e8c48984bc30";
    //请求地址
    ...
    $appId        = '8aaf070859b4c993015a06e624ca0987';
    //请求端口，生产环境和沙盒环境一致
    $serverPort   = '8883';
    ...
    include_once 'Api/Controller/RestSmsController.class.php';
    $rest = new \Api\Controller\RestSmsController($serverIP, $serverPort, $softVersion);
    ...
    if($result == NULL ) {
        ...
        exit(json_encode(['status'=>-1,'msg'=>'短信发送错误！']));
    }
    if($result->statusCode!=0) {
      //  echo "error code :" . $result->statusCode . "<br>";
       // echo "error msg :" . $result->statusMsg . "<br>";
        //TODO 添加错误处理逻辑
//             print_r(json_decode(json_encode($result),true));exit();
       // 160038【短信】短信验证码发送过频繁
        header('Content-Type:application/json; charset=utf-8');
        if($result->statusCode == 160038){
            exit(json_encode(['status'=>-1,'msg'=>"30秒内不能重复发送哦"]));
        }else{
            $result = json_decode(json_encode($result),true);
            exit(json_encode(['status'=>-1,'msg'=>$result["statusMsg"]]));
        }
    }else{
       ...
       // echo "smsMessageSid:".$smsmessage->smsMessageSid."<br/>";
        //TODO 添加成功处理逻辑
        header('Content-Type:application/json; charset=utf-8');
        return json_encode(['status'=>1,'msg'=>'短信发送成功!','time'=>$smsmessage->dateCreated]);
    }
}
...
function gethometownByIdcard($card_number){  
      $num = substr($card_number,0,6);
      $areas = C('AREAS');
      return $areas[$num];
}
/**
 ...
 * @return string|mixed
 */
function chcode_date($date = ''){
	if($date) return '';
	if(strpos('-', $date)) return str_replace('-', '-', $date);
	return $date;
}