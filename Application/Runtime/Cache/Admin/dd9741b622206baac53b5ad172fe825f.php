...
<script>
$(function(){
	var act_nav = 'course_tag';
	$('.nav-tabs').find('li[name='+act_nav+']').addClass('active');
})
</script><?php endif; ?>
	               	<nav class="navbar navbar-default">	     
				        <div class="collapse navbar-collapse">
				          <form class="navbar-form form-inline" action="<?php echo U('Course/course_tag');?>" method="get">
				            <div class="form-group">
				              	<input type="text" name="keywords" class="form-control" placeholder="标签名称" value="<?php echo I('keywords');?>">
				            </div>
				            ...
				            <div class="form-group pull-right">
					            ...
					            <input id="tag_input" style="display:none;" name="name" class="form-control" placeholder="请输入标签名称" value="<?php echo I('keywords');?>">
		              ...
		              <table id="list-table" class="table table-bordered table-striped dataTable">
						...
						<tbody>
						  <?php if(is_array($list)): foreach($list as $k=>$vo): ?><tr role="row" align="center">
                             ...
                             <td><input type="text" value="<?php echo ($vo["name"]); ?>" onblur="change_name('<?php echo ($vo["id"]); ?>',this.value)"/></td>
                             <td><?php echo ($vo["create_date"]); ?></td>
		                     <td>
			                     <a title="删除" class="btn btn-danger" href="javascript:void(0)" data-id="<?php echo ($vo["id"]); ?>" onclick="dele_course_tag('<?php echo ($vo["id"]); ?>')"><i class="fa fa-trash-o"></i></a>
							 </td>
...
function change_name(id,name){
	$.post("<?php echo U('Course/edit_course_tag');?>",{info:{id:id,name:$.trim(name)},act:'save'},function(res){
...
function dele_course_tag(id){
	jsconfirm('确认要删除吗？',function(){
		$.ajax({
			type : 'post',
			url : '<?php echo U("Course/dele_course_tag");?>',
			data : {id:id},
			dataType : 'json',
			success : function(data){
				if(data.status == '1'){
