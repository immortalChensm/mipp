...
<script>
$(function(){
	var act_nav = 'course_type';
	$('.nav-tabs').find('li[name='+act_nav+']').addClass('active');
})
</script><?php endif; ?>
	               	<nav class="navbar navbar-default">	     
				        <div class="collapse navbar-collapse">
				          <form class="navbar-form form-inline" action="<?php echo U('Course/course_type');?>" method="get">
				            <div class="form-group">
				              	<input type="text" name="keywords" class="form-control" placeholder="类型名称" value="<?php echo I('keywords');?>">
				            </div>
		              ...
		              <table id="list-table" class="table table-bordered table-striped dataTable">
						...
						<tbody>
						  <?php if(is_array($list)): foreach($list as $k=>$vo): ?><tr role="row" align="center">
		                     ...
		                     <td>
		                     	<input name="info[sort_index]" type="text" value="<?php echo ($vo["sort_index"]); ?>" onblur="change_sort('<?php echo ($vo["id"]); ?>',this.value)"/>
		                     </td>
		                     <td><img src="<?php echo ($vo["pic"]); ?>" onclick="window.open('<?php echo ($vo["pic"]); ?>')" style="cursor:pointer; height:50px;"></td>
                             <td><?php echo ($vo["name"]); ?></td>
                             <td><?php echo substr($vo['create_date'],0,10);?></td>
		                     <td>
			                     <a title="编辑" class="btn btn-primary" href="javascript:void(0)" onclick="edit_course_type('<?php echo ($vo["id"]); ?>')"><i class="fa fa-pencil"></i></a>
			                     <a title="删除" class="btn btn-danger" href="javascript:void(0)" data-id="<?php echo ($vo["id"]); ?>" onclick="dele_course_type('<?php echo ($vo["id"]); ?>')"><i class="fa fa-trash-o"></i></a>
							 </td>
...
function change_sort(id,sort_index){
	$.post("<?php echo U('Course/edit_course_type');?>",{info:{id:id,sort_index:$.trim(sort_index)},act:'save'},function(res){
		if(res.status == '1') location.reload();
	});
}
function edit_course_type(id){
	$.post("<?php echo U('Course/edit_course_type');?>",{act:'show',id:id},function(data){
		dialog(data,'',function(){
			$.post("<?php echo U('Course/edit_course_type');?>",$('#dataForm').serializeArray(),function(res){
				if(res.status == '1'){
...
function dele_course_type(id){
	jsconfirm('确认要删除吗？',function(){
		$.ajax({
			type : 'post',
			url : '<?php echo U("Course/dele_course_type");?>',
			data : {id:id},
			dataType : 'json',
			success : function(data){
				if(data.status == '1'){
