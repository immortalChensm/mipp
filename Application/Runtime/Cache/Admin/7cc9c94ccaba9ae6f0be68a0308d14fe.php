	             ...
	             <div class="box-header">
	               	<nav class="navbar navbar-default">	     
				        <div class="collapse navbar-collapse">
				          <form class="navbar-form form-inline" action="<?php echo U('Admin/Teacher/apply_list');?>" method="get">
		              ...
		              <table id="list-table" class="table table-bordered table-striped dataTable">
						...
						<tbody>
						  <?php if(is_array($list)): foreach($list as $k=>$vo): ?><tr role="row" align="center">
		                     ...
		                     <?php if($vo["status"] == 2): ?><td style="color:red;">审核不通过</td>
		                     <?php elseif($vo["status"] == 3): ?>
		                     <td style="color:orange;">待审核</td><?php endif; ?>
		                     <td><?php echo date('Y-m-d',strtotime($vo['create_date']));?></td>
		                     <td>
		                      <a title="审核通过" class="btn btn-primary" href="javascript:void(0)" onclick="check_teacher('<?php echo ($vo["id"]); ?>',1)">通过</a>
		                      <?php if($vo["status"] == 3): ?><a title="审核不通过" class="btn btn-danger" href="javascript:void(0)" onclick="check_teacher('<?php echo ($vo["id"]); ?>',2)">不通过</a><?php endif; ?>
							 </td>
...
function set_stick(obj,id,is_stick){
	$.post("<?php echo U('Teacher/set_stick');?>",{id:id,is_stick:is_stick},function(data){
		if(is_stick == '1'){
...
function check_teacher(id,status){
	$.post("<?php echo U('Teacher/check_teacher');?>",{id:id,status:status},function(res){
		if(res.status == '1'){
...
function dele_teacher(id){
	jsconfirm('确认要注销此账号吗？',function(){
		$.ajax({
			type : 'post',
			url : "<?php echo U('Teacher/dele_teacher');?>",
			data : {id:id},
			dataType : 'json',
			success : function(data){
				if(data.status == '1'){
