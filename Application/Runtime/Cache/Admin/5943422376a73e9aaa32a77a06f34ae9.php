	             ...
	             <div class="box-header">
	               	<nav class="navbar navbar-default">	     
				        <div class="collapse navbar-collapse">
				          <form class="navbar-form form-inline" action="<?php echo U('System/banner');?>" method="get">
		              ...
		              <table id="list-table" class="table table-bordered table-striped dataTable">
						...
						<tbody>
						  <?php if(is_array($list)): foreach($list as $k=>$vo): ?><tr role="row" align="center">
		                     ...
		                     <td>
		                     	<a href="<?php echo ($vo["link"]); ?>" target="_blank"><?php echo mb_strlen($vo['link'])>30?substr($vo['link'],0,30).'...':$vo['link'];?></a>
		                     </td>
		                     ...
		                     <td>
			                     <a title="编辑" class="btn btn-primary" href="javascript:void(0)" onclick="edit_banner('<?php echo ($vo["id"]); ?>')"><i class="fa fa-pencil"></i></a>
			                     <a title="删除" class="btn btn-danger" href="javascript:void(0)" data-id="<?php echo ($vo["id"]); ?>" onclick="dele_banner('<?php echo ($vo["id"]); ?>')"><i class="fa fa-trash-o"></i></a>
							 </td>
...
function change_sort(id,sort_index){
	$.post("<?php echo U('System/edit_banner');?>",{info:{id:id,sort_index:$.trim(sort_index)},act:'save'},function(res){
		if(res.status == '1') location.reload();
	});
}
function edit_banner(id){
	$.post("<?php echo U('System/edit_banner');?>",{act:'show',id:id},function(data){
		dialog(data,'添加Banner',function(){
			$.post("<?php echo U('System/edit_banner');?>",$('#dataForm').serializeArray(),function(res){
				if(res.status == '1'){
...
function dele_banner(id){
	jsconfirm('确认要删除吗？',function(){
		$.ajax({
			type : 'post',
			url : '<?php echo U("System/dele_banner");?>',
			data : {id:id},
			dataType : 'json',
			success : function(data){
				if(data.status == '1'){
