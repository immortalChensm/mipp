	             ...
	             <div class="box-header">
	               	<nav class="navbar navbar-default">	     
				        <div class="collapse navbar-collapse">
				          <form class="navbar-form form-inline" action="<?php echo U('User/index');?>" method="get">
				            <div class="form-group">
				              	<input type="text" name="keywords" class="form-control" placeholder="昵称/姓名/手机号" value="<?php echo ($request_data["keywords"]); ?>">
				              	<select name="identify" class="form-control">
				              	<option value="">会员身份</option>
				              	<option value="1" <?php echo $request_data['identify'] == 1 ? 'selected':'';?> >普通会员</option>
				              	<option value="2" <?php echo $request_data['identify'] == 2 ? 'selected':'';?> >教师</option>
				              	</select>
				            </div>
		              ...
		              <table id="list-table" class="table table-bordered table-striped dataTable">
						...
						<tbody>
						  <?php if(is_array($list)): foreach($list as $k=>$vo): ?><tr role="row" align="center">
		                     ...
		                     <td><img src="<?php echo ($vo["headimgurl"]); ?>" onclick="window.open('<?php echo ($vo["headimgurl"]); ?>')" style="cursor:pointer; height:50px;"></td>
		                     <td><?php echo ((isset($vo["nickname"]) && ($vo["nickname"] !== ""))?($vo["nickname"]):'--'); ?></td>
		                     <td><?php echo $vo['identify'] == '1'?'普通会员':'老师';?></td>
							 <td><?php echo ((isset($vo["name"]) && ($vo["name"] !== ""))?($vo["name"]):'--'); ?></td>
							 <td><?php echo ((isset($vo["phone"]) && ($vo["phone"] !== ""))?($vo["phone"]):'--'); ?></td>
		                     <td><?php echo ((isset($vo["consume_money"]) && ($vo["consume_money"] !== ""))?($vo["consume_money"]):'0'); ?></td>
		                     <td><?php echo ($vo["create_date"]); ?></td>
		                     <td>
		                       <a title="查看订单" class="btn btn-primary" href="<?php echo U('Order/user_order',array('user_id'=>$vo['id']));?>"><i class="fa fa-cart-arrow-down"></i></a>
		                       <?php if($vo["identify"] == 2): ?><a title="越权登录" class="btn btn-primary" href="<?php echo U('Admin/over_login',array('teacher_id'=>$vo['teacher_id']));?>"><i class="fa fa-user"></i></a><?php endif; ?>
							 </td>
