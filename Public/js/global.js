...
// 获取最后一级乡镇
function get_twons(t){
    var parent_id = $(t).val();
    $('#twon').empty().css('display','block');
...
// 获取cun
function get_cun(obj){
    var parent_id = $(obj).val();
    var url = '/index.php?m=Home&c=Api&a=getCun&level=5&parent_id='+ parent_id;
    $.ajax({
        type : "GET",
        url  : url,
        success: function(res) {
            if(parseInt(res) == 0){
                $('#cun').empty().css('display','none');
            }else{
